...
static lock_record_t lock_records[N_THREADS * 400]; // 400 iterations per thread
...
static bool run_lock_test(lock_ops_t *lock_ops) {
  ...
  while (global_round_counter < 400) {
  ...
  printf("Final global_round_counter: %d (target was %d)\n",
         global_round_counter, N_THREADS * 400);
